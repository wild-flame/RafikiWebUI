...
//import "hi" from 'react-icons/lib/fa/tag'
//import "hi" from 'react-icons/lib/fa/user'
...
//    master.commit(this.commitAttributes({ message: "commit #14" }))
  ...
  render() {
    return(
        <table style={{fontSize:'12px',border:'0px',borderSpacing:'0px'}}>
          <tr id={'row-' + importedData[0].short_id} style={{height:'8px'}}>
            <td rowspan={importedData.length+2}>
              <GitGraph
  ...
  getCommitMessage = (myTemplateConfig, commit) => {
    return [
      <td align="left" style={{paddingTop:'0px',paddingBottom:'0px',paddingLeft:'10px',paddingRight:'3px'}}>{commit ? commit.tag ? "hi": undefined : undefined}</td>,
      ...
      <td align="left" style={{paddingTop:'0px',paddingBottom:'0px',paddingLeft:'10px',paddingRight:'3px'}}>{commit ? "hi" : undefined}</td>,
